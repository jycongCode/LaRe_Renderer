...
RenderOptions parse_integrator(pugi::xml_node node,
    ...
    if (type == "path") {
        ...
        for(auto child : node.children()){
            std::string name = child.attribute("name").value();
            if(name == "maxM" || name == "max_M"){
                options.max_M = parse_integer(
                    child.attribute("value").value(),default_map);
            }else if(name == "initialM" || name == "initial_M"){
                options.initial_M = parse_integer(
                    child.attribute("value").value(),default_map);
            }else if(name == "n" || name == "N"){
                options.N = parse_integer(
                    child.attribute("value").value(),default_map);
            }else if(name == "k" || name == "K"){
                options.k = parse_integer(
                    child.attribute("value").value(),default_map);
            }else if(name == "spatial_iter"){
                options.spc_iter_num = parse_integer(
                    child.attribute("value").value(),default_map);
            }
        }
    }else if (type == "direct") {
