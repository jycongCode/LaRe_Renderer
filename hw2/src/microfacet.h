...
inline Real fresnel_dielectric(Real n_dot_i, Real eta) {
    ...
    // Real F0 = (1.0-eta)/(1.0+eta)*(1.0-eta)/(1.0+eta);
    // return schlick_fresnel(F0,n_dot_t);
}
...
/// This is the anistropic Trowbridge-Reitz distribution popularized by Walter
/// et al
inline Real GGX(Frame frame,Vector3 half,Real anistropic,Real roughness){
    Real aspect = sqrt(Real(1)-0.9*anistropic);
    Real alphax = max(0.0001,roughness*roughness/aspect);
    Real alphay = max(0.0001,roughness*roughness*aspect);
    Vector3 projected = normalize(to_local(frame,half));
    Real halpha = projected.x * projected.x / (alphax*alphax)+\
                    projected.y * projected.y / (alphay*alphay)+\
                     projected.z * projected.z;
    return Real(1) / (c_PI*alphax*alphay*halpha*halpha);
}
